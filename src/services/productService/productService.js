...
exports.getProductById = async (id) => {
  try {
    ...
    if (product) {
      const flattenedCategories = product.productCategories.map(
        (singleObj) => singleObj.category
      );
      const updatedProduct = {
        ...product,
        productCategories: flattenedCategories,
      };
  ...
  try {
    ...
    if (Array.isArray(data.categories) && data.categories.length > 0) {
      for (const category of data.categories) {
        ...
        await productRepository.addProductCategories(categoryRelation);
      }
    }
...
exports.getAllProductTags = async (pageNumber) => {
  try {
    ...
    return await productRepository.getProductTags(page, limit);
  } catch (error) {
    throw error;
  }
};
...
exports.createProductReview = async (authorId, data) => {
  try {
    const reviewData = {
      review: data.review,
    ...
    return await productRepository.addProductReview(reviewData);
  } catch (error) {
    throw error;
  }
};
